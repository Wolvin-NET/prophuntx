-- taken from Sandbox's laser beam
-- UNFINISHED/UNUSED
EFFECT.SparkMat = Material( "effects/spark" )
...
function EFFECT:Init( data )
	...
	if ( IsValid( ent ) && att > 0 ) then
		--if ( ent.Owner == LocalPlayer() && !LocalPlayer():GetLPSWeaponEntity():GetOwner() != LocalPlayer() ) then ent = ent.Owner:GetLPSWeaponEntity() end
